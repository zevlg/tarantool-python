...
class RequestInsert(unittest.TestCase):
    ...
    def test__cast_to_bytes(self):
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"0d0000001b00000000000000010000000000000003000000040100000004d00700000430750000")
        )

        self.assertEqual(
            bytes(tarantool.request.RequestInsert(self.conn2, 1, (b"AAA", b"BBBB", b"CCCCCC"), False)),
            binascii.unhexlify(b"0d0000001c0000000000000001000000000000000300000003414141044242424206434343434343")
        )
...
class RequestDelete(unittest.TestCase):
    ...
    def test__cast_to_bytes(self):
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"1500000011000000000000000100000000000000010000000401000000")
        )

        self.assertEqual(
            bytes(tarantool.request.RequestDelete(self.conn2, 1, b'AAA', False)),
            binascii.unhexlify(b"15000000100000000000000001000000000000000100000003414141")
        )
...
class RequestSelect(unittest.TestCase):
    ...
    def test__cast_to_bytes(self):
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"110000001d00000000000000010000000000000000000000ffff000001000000010000000401000000"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"110000001c00000000000000010000000000000000000000ffff0000010000000100000003414141"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"110000002f00000000000000010000000000000000000000ffff000003000000010000000401000000010000000402000000010000000403000000"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"110000002200000000000000010000000000000000000000ffff0000010000000200000004010000000402000000"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"110000003000000000000000010000000000000000000000ffff00000200000002000000040100000004020000000200000004030000000404000000"),
...
class RequestUpdate(unittest.TestCase):
    ...
    def test__cast_to_bytes(self):
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"130000001f0000000000000011000000000000000100000004220000000100000033000000000444332211"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"130000002100000000000000110000000000000001000000065a5a5a5a5a5a0100000033000000000444332211"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"130000001e000000000000001100000000000000010000000422000000010000003300000000034e4e4e"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"130000002000000000000000110000000000000001000000065a5a5a5a5a5a010000003300000000034e4e4e"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"130000003300000000000000110000000000000001000000042200000003000000330000000004333300004400000000044444000055000000000455550000"),
        ...
        self.assertEqual(
            bytes(tarantool.request.RequestUpdate(self.conn, 0x11, b"ZZZZZZ", [(0x33, '=', 0x3333), (0x44, '=', 0x4444), (0x55, '=', 0x5555)], False)),
            binascii.unhexlify(b"130000003500000000000000110000000000000001000000065a5a5a5a5a5a03000000330000000004333300004400000000044444000055000000000455550000"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"1300000030000000000000001100000000000000010000000422000000030000003300000000034b4b4b4400000000034c4c4c5500000000034d4d4d"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"130000003200000000000000110000000000000001000000065a5a5a5a5a5a030000003300000000034b4b4b4400000000034c4c4c5500000000034d4d4d"),
        ...
        self.assertEqual(
            ...
            binascii.unhexlify(b"130000001f00000000000000"
                               +b"11000000"   # space_no
                               +b"00000000"   # flags
                               +b"01000000"   # key cardinality
                               +b"0422000000" # key value
                               +b"01000000"   # count (number of operations)
                                              # --- operation triplets ---
                               +b"33000000"   # field_no = 0x33
                               +b"01"         # op_code = add ('+')
                               +b"0455000000" # field = 0x55
                               ),
            #
        ...
        self.assertEqual(
            bytes(tarantool.request.RequestUpdate(self.conn, 0x11, 0x22, [(0x33, '&', 0x55)], False)),
            binascii.unhexlify(b"130000001f00000000000000" # 12 byte header
                               +b"11000000"    # space_no
                               +b"00000000"    # flags
                               +b"01000000"    # key cardinality
                               +b"04220000000" # key value
                               +b"1000000"     # count (number of operations)
                                               # --- operation triplets ---
                               +b"33000000"    # field_no = 0x33
                               +b"02"          # op_code = AND ('&')
                               +b"0455000000"  # field = 0x55
