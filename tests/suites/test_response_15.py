...
class Test_Field15(unittest.TestCase):
    '''
    ...
    def test__init_from_unicode(self):
        ...
        self.assertEqual(
            tarantool.lts.response.field(value),
    ...
    def test__init_from_int(self):
        ...
        with self.assertRaises(ValueError):
            tarantool.lts.response.field(0xffffffffffffffff+1)
            tarantool.lts.response.field(-1)

        # Unsupported argument type
        with self.assertRaises(TypeError):
                tarantool.lts.response.field(None)
                tarantool.lts.response.field([1,2,3])
    ...
    def test__cast_to_int(self):
        ...
        for i in (0, 0x11, 0x1122, 0x112233, 0xffffffff, 0xffffffffffffffff):
            f = tarantool.lts.response.field(i)
            self.assertEqual(
                int(f),
                i,
        ...
        f = tarantool.lts.response.field(b"not an int value")
        with self.assertRaises(ValueError):
            int(f)
    ...
    def test__cast_to_str(self):
        ...
        if py3:
            self.assertEqual(
                str(tarantool.lts.response.field(b"\xd0\xa2\xd0\xb5\xd1\x81\xd1\x82")),
                str(b"\xd0\xa2\xd0\xb5\xd1\x81\xd1\x82", "utf-8"),
                "Cast field instance to unicode")
        else:
            self.assertEqual(
                unicode(tarantool.lts.response.field(b"\xd0\xa2\xd0\xb5\xd1\x81\xd1\x82")),
                unicode(b"\xd0\xa2\xd0\xb5\xd1\x81\xd1\x82", "utf-8"),
                "Cast field instance to unicode")



class Test_Response15(unittest.TestCase):
    '''
    ...
    def test__init_single(self):
        ...
        self.assertEqual(
            tarantool.lts.response.Response(ConnectionMock(tarantool.lts.STR), header, body),
    ...
    def test__init_attrs(self):
        ...
        r = tarantool.lts.response.Response(ConnectionMock(tarantool.lts.STR), header, body)
